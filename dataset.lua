...
-- Used for training target samples
-- converts a table of samples (w/o corresponding labels) to a clean tensor
local function tableToOutputWO(self, dataTable)
   local data
...
function dataset:sampleTarget(quantity)
    assert(quantity)
    local dataTable = {}
    local scalarTable = {}
    for i=1,quantity do
        local ind = torch.random(1, self.numSamples)
        local imgpath = ffi.string(torch.data(self.imagePath[ind]))
        local out = self:sampleHookTrainTarget(imgpath)
        table.insert(dataTable, out)
    end
    local data, scalarLabels = tableToOutputWO(self, dataTable, scalarTable)
    return data, scalarLabels
end
