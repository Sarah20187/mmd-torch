...
local trainTargetCache = paths.concat(opt.cache, 'trainTargetCache.t7')
...
local trainHook = function(self, path)
   ...
   -- mean/std
   -- mean is simply a 3-d vector
   -- the image read ranges from (0, 1)
...
testHook = function(self, path)
   ...
   -- mean/std
   -- Note: when training with the pre-trained Alex net, the input data ranges from 0 to 255. (no divided by std)
   -- Hence we mul(255) and comment out the std
   for i=1,3 do -- channels
      if mean then out[{{i},{},{}}]:add(-mean[i]) end
      out[{{i},{},{}}]:mul(255)
      -- if std then out[{{i},{},{}}]:div(std[i]) end
...
--------------------------------------------------------------------------------
--[[
   Section 3: Create a target training data loader (targetLoader)
--]]

if(opt.isDA == 1) then
   local trainTargetHook = function(self, path)
      collectgarbage()
      local input = loadImage(path)
      local iW = input:size(3)
      local iH = input:size(2)
   ...
   if paths.filep(trainTargetCache) then
      print('Loading train target metadata from cache')
      trainTargetLoader = torch.load(trainTargetCache)
      trainTargetLoader.sampleHookTrainTarget = trainTargetHook
      assert(trainTargetLoader.paths[1] == paths.concat(opt.data, 'trainTarget'),
             'cached files dont have the same path as opt.data. Remove your cached files at: '
                .. trainTargetCache .. ' and rerun the program')
   else
      print('Creating train metadata')
      trainTargetLoader = dataLoader{
         paths = {paths.concat(opt.data, 'trainTarget')},
         loadSize = loadSize,
         sampleSize = sampleSize,
         split = 100,
         verbose = true
      }
      torch.save(trainTargetCache, trainTargetLoader)
      trainTargetLoader.sampleHookTrainTarget = trainTargetHook
   end
   collectgarbage()
end
-- do some sanity checks on trainTargetLoader
-- We choose not do any checks here since sometimes #targetcls is unaware

-- End of target training data loader section
